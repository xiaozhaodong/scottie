                ...
                let (title_text, full_title) =
                    if let Some(name) = tab.name.as_ref().filter(|n| !n.trim().is_empty()) {
                        ...
                        let show_activity_prefix =
                            cx.global::<Config>().show_agent_title_activity_prefix;
                        let raw = match view.label_with_activity(show_activity_prefix) {
                            TabLabel::Osc(title) | TabLabel::Cwd(title) => {
                                abbreviate_home(strip_host_prefix(title.trim()), home.as_deref())
                                    .into_owned()
                            }
                            // A task title is prose: no `user@host:` head to cut
                            // and no path under `home` to spell as `~`.
