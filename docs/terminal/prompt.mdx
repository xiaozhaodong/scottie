...
Scottie puts an editor at the shell prompt. Nothing to install, no plugin to source
...
<Frame caption="Placeholder — video: typing a command with a ghost suggestion, accepting it, then opening the completion menu">
  <img src="/images/placeholder.svg" alt="The Scottie prompt" />
</Frame>
...
When Scottie has nothing useful to offer, the <kbd>⇥</kbd> falls through to your
...
The prompt features arrive through Scottie's shell integration, which is injected
...
Settings while it is off — both are menus Scottie opens inside that editor, so
