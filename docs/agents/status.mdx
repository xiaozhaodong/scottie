...
waiting for permission look identical from outside. Scottie fixes that by letting
...
becomes **Reinstall** — or **Update**, against an **Outdated** state, when Scottie
ships a newer hook.

<Note>
  The hooks only do anything inside Scottie. Running the same agent in another
...
Scottie keeps a status item in the system tray (menu bar on macOS). It flips to an
...
If nothing recognisable is running, Scottie says *"No running coding agent found"*
