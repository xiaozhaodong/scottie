...
further: Scottie runs a server on the far machine, and the whole workbench points
...
<Frame caption="Placeholder — screenshot: a remote workspace open, sidebar showing remote repos, with the machine name in the strip">
  <img src="/images/placeholder.svg" alt="A remote workspace in Scottie" />
</Frame>
...
<Steps>
  ...
  <Step title="Pick a machine">
    Scottie connects over the same SSH stack as everything else, so profiles,
  ...
  <Step title="Approve the server install, once">
    ...
    > Scottie will write its server binary to *devbox* so this machine can host
...
that disagree never quietly half-work — Scottie installs the matching one instead.
...
<AccordionGroup>
  <Accordion title="Update Scottie's server on “devbox”?">
    The machine is serving sessions from a build whose protocol this client
    cannot speak. Scottie has already installed a matching server, but the one
  ...
  <Accordion title="Restart Scottie's server on “devbox”?">
...
<Warning>
  Both of these end other people's work if the machine is shared. Scottie spells
...
- **Fork an agent session.** The fork command would run against the *local*
  agent, so Scottie does not offer it.
