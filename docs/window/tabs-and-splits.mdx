...
Once a tab holds more than one pane, each pane names itself along its top edge:
the title the program running in it set, or the agent working in it, or the
folder it sits in when neither has spoken. Three shells in three directories, or
three SSH panes on three machines, are told apart at a glance instead of by
counting.
...
A tab with one pane has no header, and neither does a zoomed pane
(<kbd>⌘ ⇧ ⏎</kbd>): there is nothing on screen to tell it apart from, and the
window title, the tab bar and the sidebar all name it already.

The strip costs about a line and a half of grid per pane. Turn it off with
**Settings → Appearance → Show pane titles**.

## Rearranging by dragging

The header is also the grip: drag a pane by its title to move it somewhere else
in the tab. With pane titles turned off, hover a pane instead and a small grip
appears along its top edge — the same drag, a smaller target.
...
The same move backwards. Drag a pane by its grip — its title, or the small grip
that appears on hover when pane titles are off — up to the tab bar, or out to
