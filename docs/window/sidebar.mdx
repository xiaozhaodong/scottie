...
The left sidebar is Scottie's tab bar, and it is the default because a vertical row
...
<Frame caption="Placeholder — screenshot: the sidebar with two repo groups, agent avatars and status dots, branch and +N −M counts">
  <img src="/images/placeholder.svg" alt="The Scottie tab sidebar" />
</Frame>
...
pane the other way — by its title, or by the small grip that appears on hover
when [pane titles](/window/tabs-and-splits#pane-titles) are off, onto the
sidebar — gives it a row of its own, between whichever two the caret lands
between. See
