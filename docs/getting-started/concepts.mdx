...
Four words explain most of Scottie. Three of them you can see; the fourth is the
reason the other three survive a reboot.

## Pane

A **pane** is one terminal: one shell (or one program) attached to one PTY. It
is the only thing in Scottie that actually runs something.
...
label is the best evidence Scottie has: a name you set, else the coding agent
...
Workspaces are how Scottie keeps ten repositories from becoming forty
...
<Frame caption="Placeholder — screenshot: the workspace switcher, workspaces on the left, that workspace's tabs on the right">
  <img src="/images/placeholder.svg" alt="The Scottie workspace switcher" />
</Frame>
...
Quitting Scottie closes the window and leaves that server running. Your build keeps
building, your agent keeps working, your SSH session stays up. Open Scottie again
...
| | Close a tab | Quit Scottie | Stop the server | Reboot |
|---|:--:|:--:|:--:|:--:|
...
`echo $TTY7_PANE` is the fastest way to tell whether you are inside Scottie at all.
