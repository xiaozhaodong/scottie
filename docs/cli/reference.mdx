...
Set inside every Scottie pane, inherited by anything launched from one.
...
Outside a Scottie shell, address-taking verbs fail with
...
to do. `--enter` is that key only for the `%`-marked spelling: `Scottie send 83
